...
SEED = 42  # same seed as ch.16 → identical 10 windows
...
def fit_one_channel_joint(
    ...
    # Reference Personalized GP test NLL — same recipe as ch.16 → directly comparable
    ...
    # Train NLL of joint Hawkes prediction (and Personalized GP reference for symmetry)
    ...
    # Test prediction: λ_u · b_t + states · α  (λ_u = 1 for unseen users)
...
def main() -> None:
    ...
    # === mean(λ_u) per channel distribution ===
    ...
    # === Side-by-side compare with ch.16 (Scaled-baseline) ===
