...
<!--
Тот же функциональный вид. Новое — оцениваем 9 коэффициентов одновременно, чтобы получить структуру всех кросс-переходов между каналами.
-->
...
- Для диагональных: $\mathbb{E}[z_{u,t}^{(k)}] \propto \lambda_u^{(k)} \cdot b_t$ — тот же сигнал, что и baseline. Линейная комбинация двух почти коллинеарных регрессоров → ridge-подобный «жёлоб» в parameter space.
