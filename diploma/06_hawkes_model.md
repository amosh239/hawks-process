...
Ниже в разделе `6.11` отдельно показано, что именно компактный базис `(1,3)` выбирается как основной по итогам сравнения нескольких наборов half-life.
...
Это уже качественно другой результат по сравнению с preliminary additive-версией:

1. likelihood-метрики заметно улучшаются;
2. `MAE` и `RMSE` тоже улучшаются;
3. положительный bias исчезает.

## 6.6. Что изменилось по сравнению с предварительной additive-версией

Главная ценность новой постановки видна именно в прямом сравнении с ранним additive Hawkes.

### Test: additive Hawkes против новой модели из главы 06

| Metric | Additive Hawkes | Scaled-baseline Hawkes | Delta new - old |
| --- | ---: | ---: | ---: |
| `poisson_loglik` | `-205827.62` | `-203093.06` | `+2734.56` |
| `mean_poisson_nll` | `0.40113` | `0.39580` | `-0.00533` |
| `mean_poisson_deviance` | `0.62992` | `0.61926` | `-0.01066` |
| `MAE` | `0.22591` | `0.21691` | `-0.00900` |
| `RMSE` | `0.63011` | `0.62719` | `-0.00292` |
| `relative_aggregate_bias` | `+7.05%` | `-2.00%` | `-9.05 pp` |

Scale-параметр и более компактный выбор half-life вместе снимают главное методологическое слабое место ранней additive-версии: Hawkes перестает работать как механизм безусловного поднятия baseline.

## 6.7. Дневная динамика

![Scaled-baseline Hawkes daily aggregate](reports/experimental_1_hawkes/daily_aggregate_analysis_window.png)

По графику видно, что выбранная версия Hawkes не уводит среднюю интенсивность систематически вверх. При этом локальные отклонения от baseline сохраняются и дают прирост по likelihood.

## 6.8. Какие сигналы реально использовались
...
4. По сравнению с ранним additive Hawkes качество тоже заметно лучше.
5. Поэтому дальше именно эта постановка используется как основной Hawkes-кандидат, а выбор числа half-life фиксируется внутри раздела `6.11`.

## 6.11. Нужны ли более длинные half-life
